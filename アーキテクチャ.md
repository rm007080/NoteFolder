# アーキテクチャ設計書：NoteFolder（v6更新版）

本ドキュメントは、NoteFolder Chrome拡張機能の技術的な設計思想と構造を記述する。

> **更新履歴**:
> - 2025-12-31 v6更新版: タグ階層化、D&D機能、タグ統合機能を追加
> - 2025-12-26 v3修正版の実装計画に基づき更新
...
| 機能 | 状態 | 備考 |
|------|------|------|
| タグの色分け | **実装済み** | storage に `tagColors` オブジェクト追加 |
| タグの階層化 | **実装済み** | `親/子` 形式でパス表記（v6実装） |
| タグのD&D | **実装済み** | ドラッグ&ドロップで親子関係変更（v6実装） |
| タグの統合 | **実装済み** | 同名タグへのD&Dで自動統合（v6実装） |
...
ポップオーバーとドロップダウンの両方で統一されたD&D動作を実現するため、`data-dragging-tag`属性を使用。

```javascript
// ドラッグ開始時
popover.setAttribute('data-dragging-tag', tagName);
...
async function mergeTagsInAllProjects(sourceTag, targetTag) {
  // 全プロジェクトで sourceTag を targetTag に置換
  // 重複は自動排除
}
```
...
7. **ドラッグ&ドロップ**（v6追加）
   - ポップオーバー内で親タグ→親タグのD&D
   - ポップオーバー内で子タグ→親タグのD&D
   - 自分自身へのD&Dがドロップ不可
   - 自分の子孫へのD&Dがドロップ不可
   - 同名タグへのD&Dで自動統合
...
**最終更新**: 2025-12-31（v6更新版）
**対応実装計画**: 実装計画_popover_DnD修正_v6.md
