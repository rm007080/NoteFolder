...
コード生成、セットアップ手順、ライブラリ/APIドキュメントが必要な場合は、明示的な指示がなくても**自動的にContext7 MCPツール**を使用すること。

**自動使用の対象**:
- コード生成が必要な場合
- セットアップや設定手順が必要な場合
- ライブラリやAPIのドキュメントが必要な場合

**使用手順**:
1. `mcp__context7__resolve-library-id` でライブラリIDを解決
2. `mcp__context7__get-library-docs` でドキュメントを取得

ユーザーが「use context7」と明示的に指示する必要はない。Context7が利用可能な状況では、プロアクティブに使用すること。
